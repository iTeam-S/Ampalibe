...
class AdminUser(ModelView, model=AdminModel):
    ...
    async def after_model_change(self, data, model, is_created):
        with Session(engine) as session:
            ...
            admin_model.password = hashlib.sha256(data.get('password').encode()).hexdigest()
...
class AdminAuth(AuthenticationBackend):
    async def login(self, request: Request) -> bool:
        ...
        with Session(engine) as session:
            statement = select(AdminModel).where(AdminModel.username == username)
            statement = statement.where(AdminModel.password == hashlib.sha256(password.encode()).hexdigest())
            results = session.exec(statement)
...
def init_admin(app):
    ...
    admin = Admin(app, engine, title='Ampalibe Admin', logo_url=AMPALIBE_LOGO, authentication_backend = AdminAuth(secret_key="..."))
    for view in views:
        admin.add_view(view)
